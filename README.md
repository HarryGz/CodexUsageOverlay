...
`—` means a usable value is missing, unreadable, invalid, or unsupported; it never means zero. Account and context failures are independent. A read failure for the same context retains its last known value with a stale label and original timestamp. A changed task or provenance, or compaction without a fresh token snapshot, clears the previous context value. Values older than five minutes are stale. Details include local reset times, used/remaining tokens, and each source's last successful update and age; visible countdowns and ages refresh every 15 seconds.

When local IPC has no selected route, the app chooses a recent compatible rollout and labels its context **可能非当前任务** (“may not be the current task”). More than one active IPC route also carries this label in both compact and expanded views: arrival order cannot identify the foreground window, including focus changes that produce no broadcast. A single active route is selected normally, and a remembered route survives an unfollow event until IPC disconnects or changes selection.
...
- **Session log missing or no token event:** the task may have no compatible local rollout yet, may be remote, or may not have produced a complete token-count event. While Codex is foreground, missing/rotated logs are rediscovered after 1, 2, 5, then at most every 15 seconds. Refresh re-resolves immediately and chooses a newer fallback if available. Confirm the intended `CODEX_HOME`. Context limits are not guessed from a model name.
...
- **Stale values:** context updates follow log writes; account quota polls only while Codex is foreground. Use Refresh. Empty or invalid quota updates do not advance freshness; authoritative reads replace absent windows, while valid sparse notifications preserve omitted windows. Same-context read errors preserve the last known value and original timestamp. Task/provenance changes and compaction invalidate old context values.
